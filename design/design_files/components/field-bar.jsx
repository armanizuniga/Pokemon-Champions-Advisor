// Field state controls — vertical layout for middle column
...
function FieldBar({ field, onUpdate }) {
  return (
        ...
        <div className="seg-control">
          {WEATHER_OPTS.map(w => (
            ...
            >
              ...
              <span className="seg-text">{w.label}</span>
            </button>
          ))}
        </div>
      </div>
...
function FieldToggle({ label, active, onClick }) {
  return (
    <button className={`field-toggle ${active ? "on" : ""}`} onClick={onClick}>
      <span className="toggle-dot"></span>{label}
    </button>
  );
}

function SideStateBar({ side, state, onUpdate, label }) {
  return (
...
Object.assign(window, { FieldBar, SideStateBar, FieldToggle });
